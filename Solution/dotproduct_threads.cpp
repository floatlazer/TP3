...
double dot( std::vector<double>& u, std::vector<double>& v )
{
  ...
  double scal = 0.;
  for (int i = 0; i < u.size(); ++i ) scal += u[i]*v[i];
  return scal;
}
...
int main( int nargs, char* vargs[])
{
 ...
 int sz = nbSamples/num_cpus;
 int reste = nbSamples%num_cpus;
  if ( reste > 0 ) sz += 1;
  int s = 0;
  ...
  for ( unsigned p = 0; p < num_cpus-1; ++p ) {
      ...
      threads.push_back(std::thread( [&result,&U,&V,p] (int start, int sz) -> void {
	  for( int i = start; i < start+sz; ++i )
	    result[i] = dot(U[i],V[i]);
	  }, s, size));
      s += size;
  }
  for ( int iSample = s; iSample < nbSamples; ++iSample )
